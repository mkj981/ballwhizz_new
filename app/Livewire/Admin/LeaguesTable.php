...
class CardsWeeksTable extends Component
{
    ...
    /** 💾 Update record (only if league has cards_status=1) */
    public function update()
    {
        $this->validate();

        // ✅ Ensure league is active before updating
    ...
    /** 🆕 Create new record (only if league has cards_status=1) */
    public function create()
    {
        $this->validate();

        // ✅ Ensure selected league is active
        $league = Leagues::activeCards()->find($this->league_id);
        if (!$league) {
